...
function problemA () {
  ...
  .then((stanza1)=>blue(stanza1)) // muestra el text en azul
  // .then(blue) ---> tambien se puede hacer de esta manera
...
function problemB () {
  ...
  //imprimira la que termine primero, si vamos al untils.js usan el setTimeout 
  //con un tiempo estimado con Mathrandom, es decir, que cada vez que corramos 
  //el test puede que termine uno primero que otro
...
function problemC () {
  ...
  //Forma 1: promise hell , nada prolijo si son muchas anidaciones
  // promisifiedReadFile('poem-one/stanza-02.txt')
  // .then(stanza2 => {
  //   blue(stanza2)
  ...
  //Forma 2: promesa mas prolija para muchas anidaciones
  promisifiedReadFile('poem-one/stanza-02.txt')
  .then((stanza2)=> {
    blue(stanza2);
    return promisifiedReadFile('poem-one/stanza-03.txt');
  })
  .then((stanza3)=> { //este then tomara lo que devuleva el return del primer then
    blue(stanza3);
    console.log('done');
  })
}
...
function problemD () {
  ...
  //.then(null, (err)=> magenta(new Error(err)))---> o de esta manera quitando el segundo de arriba
}
...
function problemF () {
  ...
  .then(stanza3 => {
    blue(stanza3)
    return promisifiedReadFile('poem-one/stanza-04444444.txt')
  })
  .then(stanza4 => {
    blue(stanza4)
    //console.log('done');
  ...
  .finally(()=> console.log('done'))// sea cual sea el quemino que se vaya (sea la data o el error)
  //consologuear 'done'
